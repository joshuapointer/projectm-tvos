...
auto GLResolver::ProbeCurrentContext(const ResolverState& state) -> CurrentContextProbe
{
    ...
    // tvOS/iOS uses EAGL, which has no CGLGetCurrentContext equivalent and no
    // dlsym'able currentContext symbol (the Objective-C class method
    // +[EAGLContext currentContext] is not resolvable without an ObjC bridge).
    //
    // When a user resolver is supplied via projectm_create_with_opengl_load_proc,
    // the caller contract is that they set an EAGL context current on this thread
    // before invoking create. Trust that contract: mark "some Apple GL context is
    // current" so HasCurrentContext, DetectBackend, and VerifyBackendIsCurrent all
    // accept it. The CGL slot is reused to avoid growing the Backend enum.
