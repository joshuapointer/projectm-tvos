...
auto GladLoader::CheckGLRequirements() -> bool
{
    ...
    // Apple EAGL on tvOS/iOS tops out at GLES 3.0 / GLSL ES 3.00. Apple never
    // shipped GLES 3.1 or 3.2 on EAGL, so the upstream 3.2/3.20 floor would reject
    // every Apple TV / iPhone device. Guarded so desktop GLES builds are unaffected.
    glCheck
